...
describe 'activitiesのテスト' do
  ...
  describe '投稿画面のテスト(ログイン済み)' do
    ...
    context '投稿失敗のテスト: titleを空にする' do
      before do
        @content = Faker::Lorem.characters(number: 20)
        @activity_on = Faker::Date.between(from: 2.days.ago, to: Date.today)
        select team.name, from: 'activity[team_id]'
        fill_in 'activity[content]', with: @content
        fill_in 'activity[activity_on]', with: @activity_on
        choose '公開'
      end
      
      it '投稿が保存されない' do
        expect { click_button '投稿' }.not_to change(Activity.all, :count)
      end
      it '新規投稿フォームの内容が正しい' do
        expect(find_field('activity[title]').text).to be_blank
        expect(page).to have_field 'activity[content]', with: @content
        expect(page).to have_field 'activity[activity_on]', with: @activity_on
      end
  ...
  describe '自分の投稿編集画面のテスト' do
    ...
    context '編集失敗のテスト: titleを空にする' do
      before do
        fill_in 'activity[title]', with: ''
        click_button '更新'
      end
      
      it '投稿が更新されない' do
        expect(activity.reload.title).to eq activity.title
      end
